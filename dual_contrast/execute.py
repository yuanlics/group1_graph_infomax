...
parser.add_argument('--weight',           type=float,         default=0.0,               help="weight of the 2nd term")
...
for epoch in range(nb_epochs):
    ...
    # print(features.shape, shuf_fts.shape)
    lbl_1 = torch.ones(batch_size, nb_nodes)
    lbl_2 = torch.zeros(batch_size, nb_nodes)
    lbl = torch.cat((lbl_1, lbl_2), 1)

    lbl_1_cor = torch.ones(batch_size, aug_features1.shape[1])
    lbl_2_cor = torch.ones(batch_size, aug_features2.shape[1])
    lbl_cor = torch.cat((lbl_1_cor, lbl_2_cor), 1)
    ...
    weights = [1.0, args.weight]
    if isinstance(logits, tuple):
        loss = 0.
        for ii, logit in enumerate(logits):
            loss += weights[ii] * b_xent(logit, lbls[ii])
    else:
        loss = b_xent(logits, lbl)
    # loss_a = b_xent(logits_a, lbl)
    # loss_b = b_xent(logits_b, lbl)
...
print('-' * 100)
print(args.drop_percent, args.weight)
